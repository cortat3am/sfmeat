...
async function CheckEcard() {
	return new Promise(async (resolve) => {
	  $.eCardTotal = 0
	  const options = {
		url: `https://mygiftcard.jd.com/giftcard/queryGiftCardItem/app?source=JDM`,
		body: `pageNo=1&queryType=1&cardType=-1&pageSize=200`,
		headers: {
		  ...
		  Referer: `https://mygiftcard.jd.com/giftcardForM.html?source=JDM&sceneval=2&jxsid=16517172696421820501&appCode=ms0ca95114`,
		  ...
		  if (err) {
			...
			if (safeGet(data)) {
			  ...
			  if (data.code === 'success' && data.couponVOList) {
				data.couponVOList.forEach((item) => {
				  $.eCardTotal += item.balance
				})
				$.ECardinfo = '共' + data.couponVOList.length + '张E卡,合计' + $.eCardTotal.toFixed(2) + '元';
			  }
			}
		  }
		} catch (e) {
